...
Un sistema que mueve dinero y stock necesita que *esas* operaciones sean intocables. El resto puede tener menos red.
...
test('el precio lo decide el servidor, no el cliente', async () => {
  ...
  const item = await prisma.saleItem.findFirst({ where: { saleId: res.body.id } })
...
test.each(MATRIZ)('%s %s respeta los permisos', async (ruta, metodo, ...esperados) => {
  for (const [i, rol] of ROLES.entries()) {
    expect((await pedir(ruta, metodo, await sesionDe(rol))).status).toBe(esperados[i])
  }
})
```
...
export default [
  ...next(), ...ts.configs.recommendedTypeChecked,
  {
    rules: {
  ...
  },
  {
    ...
    rules: { 'no-restricted-imports': ['error', { patterns: ['**/lib/prisma', '@prisma/client'] }] },
  },
]
```
  ...
  verificar:
    ...
    steps:
      ...
      - run: npx prisma migrate deploy       # contra la base de test
        env: { DATABASE_URL: postgresql://postgres:test@localhost:5432/kiosco_test }
      - run: npm test -- --coverage
        env: { DATABASE_URL: postgresql://postgres:test@localhost:5432/kiosco_test, JWT_SECRET: test-secret }
      - run: npm run build
