...
# Scraper auxiliar de OpenFoodFacts: alimenta el catalogo de productos por codigo
# de barras. Usa una base propia (OPENFOOD_*), independiente de la base del kiosco.
# Las credenciales se leen del entorno; ver .env.example.
conn = psycopg2.connect(
    dbname=os.environ.get("OPENFOOD_DB_NAME", "openfood"),
    user=os.environ.get("OPENFOOD_DB_USER", "postgres"),
    password=os.environ["OPENFOOD_DB_PASSWORD"],
    host=os.environ.get("OPENFOOD_DB_HOST", "localhost"),
    port=os.environ.get("OPENFOOD_DB_PORT", "5432"),
)
cursor = conn.cursor()
